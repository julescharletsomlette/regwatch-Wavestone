...
function drawMap(host, legendHost, counts){
  ...
  /* `hit`, et non `t` : la cible du survol portait le meme nom que la fonction
     de traduction, qu'elle masquait donc dans la portee du gestionnaire. Le
     jour ou « Last update » est passe par t("map.lastUpdate"), l'appel a vise
     l'element SVG au lieu de la fonction, et l'infobulle de la carte a cesse
     d'apparaitre : l'exception tombait pendant l'evaluation du gabarit, avant
     que showTip ne soit atteint. Les trois gestionnaires sont renommes, pas
     seulement celui qui traduisait, pour que le piege ne se retende pas. */
  ...
  svgEl.addEventListener("keydown", e => {
    const hit = e.target.closest(".ctry");
    if (hit && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); location.hash = "#/country/" + hit.dataset.iso; }
  });
